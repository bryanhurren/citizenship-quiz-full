...
import { Badge, TypingIndicator, UpgradeModal, Header } from '../components';
...
export const QuizScreen = () => {
  ...
  const handleSubmitAnswer = async () => {
    ...
    if (!isPartialRetry && !canUserAnswer()) {
      // Use window.confirm for web instead of Alert
...
`.trim();
  ...
  const handleUpgradeConfirm = async () => {
    ...
    try {
      ...
      // Redirect will happen, so we don't need to close the modal
    } catch (error) {
